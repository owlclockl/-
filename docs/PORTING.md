...
* `src/view/world_map.gd` + `world_map.tscn` — экран генерации: сид, прогресс, слои карты
  (природа / государства / религии / зоны / высоты), реки, города, названия, зум и панорама,
  панель «Атлас» с данными клетки под курсором и сохранение слоя в PNG.
  Открывается из мини-карты клавишей **M** или кнопкой «КАРТА МИРА».
* `src/view/map_atlas.gd` — атлас: растеризация пакованного графа в одну текстуру
  (по полосе пикселей на слой) плюс маска id клеток для выбора мышью.
...
* **Массивы.** `Uint8Array`/`Uint16Array` → `PackedByteArray`/`PackedInt32Array`.
  Важно: изменённые копии записываются назад в словарь графа (`cells["h"] = h` и т.п.) —
  документация Godot предупреждает, что значение упакованного массива, полученное из свойства или
  контейнера, является копией. По той же причине цвета атласа лежат в одном плоском
  `PackedByteArray` как поле класса и пишутся в него по индексу, а не в словаре
  `слой → массив`: `var t = _tables[layer]` дало бы копию, и `_put()` затирал бы её молча.
* **Атлас вместо полигонов.** `_draw()` у `MapRenderer` не перерисовывает клетки: `MapAtlas`
  один раз растеризует их сканлайном в изображение (`LEVELS` — пикселей на единицу карты,
  1×…3×, с бюджетом `MAX_ATLAS_PIXELS`), и дальше смена слоя — это `draw_texture_rect_region`
  по полосе этого изображения, а зум и панорама — один прямоугольник. Тот же проход оставляет
  маску id клеток, поэтому «что под курсором» — это один пиксельный поиск, а не ближайший site.
  Берега, реки, границы, города и подписи остаются вектором: только им нужен чистый пиксель
  на любом масштабе. Выпечка разбита на шаги (`bake_step`), чтобы прогресс-бар жил.
...
* Синтаксис и стиль всех `.gd`: `gdparse` + `gdlint` (gdtoolkit), конфигурация — `gdlintrc`.
  Переменные, читаемые из словарей и массивов, анонтируются явно — так скрипты не дают
  предупреждений `INFERENCE_ON_VARIANT` (в Godot 4 `:=` на значении типа Variant — ошибка).
...
* Атлас собирается на CPU и потому проверяется без рендера: `tests/test_generation.gd` печёт его
  вхолостую и сверяет размер полос, выбор клетки по маске (`cell_at`), разницу цветов слоя
  «природа» и слоя «государства», неизменность байтов при повторной выпечке и запись PNG.
* В песочнице нет исполняемого Godot, поэтому автотест `tests/test_generation.gd` нужно запустить
  ...
  инвариантов графа, климата, биомов, культур, городов, государств, дорог, детерминизма по сиду
  и checks атласа.
